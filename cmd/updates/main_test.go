...
func TestApplyUpdatesIfNeeded(t *testing.T) {
	...
	tests := []struct {
		...
		boardStatus        *UpdateStatus
		applyOsError       bool
		applyBoardError    bool
		wantOsCalled       bool
		wantBoardCalled    bool
		wantReconnectCount int
		wantError          bool
	}{
		{
			name: "Both up to date - no updates",
			osStatus: UpdateStatus{
				Installed: "7.14.1",
				Available: "7.14.1",
			},
			...
			wantOsCalled:       false,
			wantBoardCalled:    false,
			wantReconnectCount: 0,
			wantError:          false,
		},
		{
			name: "OS needs update - success",
			osStatus: UpdateStatus{
				Installed: "7.14.0",
				Available: "7.14.1",
			},
			...
			wantOsCalled:       true,
			wantBoardCalled:    false,
			wantReconnectCount: 1,
			wantError:          false,
		},
		{
			name: "Board needs update - success",
			osStatus: UpdateStatus{
				Installed: "7.14.1",
				Available: "7.14.1",
			},
			...
			wantOsCalled:       false,
			wantBoardCalled:    true,
			wantReconnectCount: 1,
			wantError:          false,
		},
		{
			name: "Both need updates - success",
			osStatus: UpdateStatus{
				Installed: "7.14.0",
				Available: "7.14.1",
			},
			...
			wantOsCalled:       true,
			wantBoardCalled:    true,
			wantReconnectCount: 2,
			wantError:          false,
		},
		{
			name: "OS update fails",
			osStatus: UpdateStatus{
				Installed: "7.14.0",
				Available: "7.14.1",
			},
		...
		{
			name: "Virtualized router - OS needs update",
			osStatus: UpdateStatus{
				Installed: "7.14.0",
				Available: "7.14.1",
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			osUpdateCalled := false
			boardUpdateCalled := false
			reconnectCount := 0

			// Create initial mock SSH connection
			mockSsh := &MockSshRunner{
				RunFunc: func(command string) (string, error) {
					if command == "/system/reboot" {
						boardUpdateCalled = true
						if tt.applyBoardError {
							return "", fmt.Errorf("board update failed")
						}
						return "", nil
					}
					if command == "/system/package/update/install" {
						osUpdateCalled = true
						if tt.applyOsError {
							return "", fmt.Errorf("os update failed")
						}
						return "", nil
					}
					// Simulate check-for-updates response
					if command == "/system/package/update/check-for-updates" {
						return `       channel: stable
  installed-version: 7.14.1
   latest-version: 7.14.1
        status: System is already up to date`, nil
					}
					if command == "/system/routerboard/print" {
						return `current-firmware: 7.14.1
upgrade-firmware: 7.14.1`, nil
					}
					return "", fmt.Errorf("unexpected command: %s", command)
				},
			}

			// Mock the reconnection factory
			sshConnectionFactory = func(host, user, password string) (core.SshRunner, error) {
				reconnectCount++
				// Always succeed in reconnection for these tests
				// Return a new mock for post-update connection
...
upgrade-firmware: 7.14.1`, nil
			...
			// Call the function
			err := applyUpdatesIfNeeded(mockSsh, cfg, tt.osStatus, tt.boardStatus)

			// Verify expectations
			if (err != nil) != tt.wantError {
				t.Errorf("applyUpdatesIfNeeded() error = %v, wantError %v", err, tt.wantError)
			}

			if osUpdateCalled != tt.wantOsCalled {
				t.Errorf("OS update called = %v, want %v", osUpdateCalled, tt.wantOsCalled)
			}

			if boardUpdateCalled != tt.wantBoardCalled {
				t.Errorf("Board update called = %v, want %v", boardUpdateCalled, tt.wantBoardCalled)
			}

			if reconnectCount != tt.wantReconnectCount {
				t.Errorf("Reconnect count = %v, want %v", reconnectCount, tt.wantReconnectCount)
			}
		})
	}
}
